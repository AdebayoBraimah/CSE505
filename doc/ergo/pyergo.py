...
def pyergo_start_session(xsb_archdir, ergo_root):
    ...
    pyxsb_start_session(
        xsb_archdir, ["--noprompt", "--quietload", "--nofeedback", "--nobanner"]
    )
...
class PYERGOException(Exception):
    ...
    def __repr__(self):
        if not self.command == None:
            if self.code == XSB_FAILURE:
                return 'PYERGOException(command=%s,message="Ergo Command failed")' % (
                    self.command
                )
            elif self.code == XSB_ERROR:
                return "PYERGOException(command=%s,message=%s)" % (
                    self.command,
                    self.message,
                )
            elif self.code == XSB_OVERFLOW:
                return (
                    "PYERGOException(command=%s,message='Overflow in XSB command')"
                    % (self.command)
                )

        elif not self.query == None:
            if self.code == XSB_ERROR:
                return "PYERGOException(query=%s,message=%s)" % (
                    self.query,
                    self.message,
                )
            elif self.code == XSB_OVERFLOW:
                return "PYERGOException(query=%s,message='Overflow in XSB query')" % (
                    self.query
                )
        else:  # not a query or command
...
class PROLOGFunctor:
    ...
    def __repr__(self):
        return "PROLOGFunctor(name=%s, module=%s, args=%s)" % (
            self.name,
            self.module,
            repr(self.args),
        )
...
def pyergo_command(cmd):
    ...
    if rcode == XSB_FAILURE:
        raise PYERGOException(
            code=rcode, command=cmd, message="Ergo Command %s failure." % (cmd)
        )
    elif rcode == XSB_ERROR:
        raise PYERGOException(
            code=rcode,
            command=cmd,
            message="Ergo Command %s error: %s %s"
            % (cmd, xsb_get_error_type(), xsb_get_error_message()),
        )
    elif rcode == XSB_OVERFLOW:
        raise PYERGOException(
            code=rcode, command=cmd, message="Ergo Command %s overflow." % (cmd)
        )
    ...
    if cmd_exception != "normal":
        raise PYERGOException(
            code=XSB_ERROR,
            command=cmd,
            message="Exception in Ergo Command %s: %s" % (cmd, cmd_exception),
        )
...
def hilog_term2py(term, auto_string=True):
    ...
    if is_float(term):
        ...
        if (
            name_maybe == XSBMODULE_FUNCTOR
            and p2c_arity(term) == 2
            and is_atom(xsb_module)
        ):
            term = p2p_arg(term, 2)
        ...
        if name_maybe == HILOG_WRAP:
            ...
            raise PYERGOException(
                message="Cannot return Ergo terms of this form to Python: %s. Only Prolog or HiLog terms can be returned."
                % print_pterm_fun(term).decode("utf8")
            )
        else:
            args = args_maybe
    ...
    raise PYERGOException(
        message="failed to detect datatype of %s" % print_pterm_fun(term).decode("utf8")
    )
...
def pyergo_query(query, auto_string=True):
    ...
    real_query = (
        "flora_call_string_command2prolog('"
    ...
    if rcode == XSB_ERROR:
        raise PYERGOException(
            code=rcode,
            query=query,
            message="Ergo query: %s error: %s %s"
            % (query, xsb_get_error_type(), xsb_get_error_message()),
        )
    elif rcode == XSB_OVERFLOW:
        raise PYERGOException(
            code=rcode, query=query, message="Ergo query: %s overflow." % (query)
        )
...
def check_active_ergo_session():
    ...
    if not ergo_session_active:
        raise PYERGOException(
            message="No active Ergo session -- call pyergo_start_session() first."
        )
