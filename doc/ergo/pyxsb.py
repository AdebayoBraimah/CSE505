...
# NOTE: Taken directly from the ErgoAI source code
...
def load_xsb_library(arch_dir=None):
    global libxsb, reg_term, c2p_int, c2p_float, c2p_string, c2p_list, c2p_nil, ensure_heap_space, c2p_functor, c2p_functor_in_mod, c2p_setfree, c2p_chars, p2c_int, p2c_float, p2c_string, p2c_functor, p2c_arity, p2c_varnum, p2c_chars, p2p_arg, p2p_car, p2p_cdr, p2p_new, p2p_unify, p2p_call, p2p_funtrail, p2p_deref, is_var, is_int, is_float, is_string, is_atom, is_list, is_nil, is_functor, is_charlist, is_attv, c2p_term, p2c_term, xsb_init, xsb_init_string, pipe_xsb_stdin, writeln_to_xsb_stdin, xsb_query_save, xsb_query_restore, xsb_command, xsb_command_string, xsb_query, xsb_query_string, xsb_query_string_string, xsb_query_string_string_b, xsb_next, xsb_next_string, xsb_next_string_b, xsb_get_last_answer_string, xsb_close_query, xsb_close, xsb_get_init_error_message, xsb_get_init_error_type, xsb_get_error_message, xsb_get_error_type, print_pterm_fun, p_charlist_to_c_string, c_string_to_p_charlist, c_bytes_to_p_charlist
    ...
    c_string_to_p_charlist.argtypes = [
        c_char_p,
        c_prolog_term,
        c_int,
        c_char_p,
    ...
    c_bytes_to_p_charlist.argtypes = [
        c_char_p,
        c_size_t,
        c_prolog_term,
        c_int,
...
class PYXSBException(Exception):
    ...
    def __str__(self):
        if not self.command == None:
            if self.code == XSB_FAILURE:
                ...
                return "XSB Command %s error(%d): %s %s" % (
                    self.command,
                    self.code,
                    self.type,
                    self.message,
            ...
            if self.code == XSB_ERROR:
                return "XSB Query %s error(%d): %s %s" % (
                    self.query,
                    self.code,
                    self.type,
                    self.message,
    ...
    def __repr__(self):
        if not self.command == None:
            if self.code == XSB_FAILURE:
                return (
                    'PYXSBException(command=%s,code=%d,type="command_failure",message="XSB Command failed")'
                    % (self.command, self.code)
                )
            elif self.code == XSB_ERROR:
                return "PYXSBException(command=%s,code=%d,type=%s,message=%s)" % (
                    self.command,
                    self.code,
                    self.type,
                    self.message,
                ...
                return (
                    "PYXSBException(command=%s,code=%d,type='overflow',message='Overflow in XSB command')"
                    % (self.command, self.code)
                )

        elif not self.query == None:
            if self.code == XSB_ERROR:
                return "PYXSBException(query=%s,code=%d,type=%s,message=%s)" % (
                    self.query,
                    self.code,
                    self.type,
                    self.message,
                ...
                return (
                    "PYXSBException(query=%s,code=%d,type='overflow',message='Overflow in XSB query')"
    ...
    def __str__(self):
        ...
        if is_python3:
            ...
            return (
                self.full_name
                + "("
                + ",".join(map(lambda a: unicode(a), self.args))
                + ")"
            )

    def __repr__(self):
        return "XSBFunctor(name=%s, module=%s, args=%s)" % (
            self.name,
            self.module,
            repr(self.args),
        )
...
def pyxsb_start_session(xsb_arch_dir_arg=None, other_args=[]):
    ...
    if not xsb_arch_dir_arg:
        ...
        try:
            xsb_arch_dir = subprocess.check_output(
                [
                    "xsb",
                    "--noprompt",
                    "--quietload",
                    ...
                    "--nobanner",
                    "-e",
                    "xsb_configuration:xsb_configuration(config_dir,_Dir), write(_Dir), halt.",
                ]
    ...
    if not xsb_arch_dir:
        raise Exception(
            "XSB autodetection failed and no architecture directory was passed to pyxsb_start_session()"
        )
    ...
    check_session_parameter(
        xsb_arch_dir_forw, "/lib/xsb_configuration.xwam", "XSB architecture"
    )
...
def pyxsb_command(cmd):
    ...
    if rcode == XSB_FAILURE:
        ...
        raise PYXSBException(
            command=cmds,
            code=rcode,
            type=xsb_get_error_type(),
            message=xsb_get_error_message(),
        )
...
def xsb_term2py(term, auto_string=True):
    ...
    #    return XSBString(value=p2c_string(term).decode('utf8',errors='ignore'))
        ...
        if auto_string and int_only and res:
            if is_python3:
                s = "".join(map(lambda b: chr(b), res))
            else:
                s = "".join(map(lambda b: unichr(b), res))
    ...
    raise Exception(
        "failed to detect datatype of %s" % print_pterm_fun(term).decode("utf8")
    )
...
def check_session_parameter(param, additional_file, param_type):
    ...
    if (not os.path.isdir(param)) or (not os.path.exists(param + additional_file)):
